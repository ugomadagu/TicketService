...
* The the number of seats in the venue will not change. If we want to change the number of seats in a venue, we will create a new TicketServiceImpl instance.
* All seats in a row are equally valuable. Meaning, seats in the front row that are on the far left or far right of the stage are just as valuable as front row seats in the middle of the stage. Therefore, it makes the most sense to fill seats from left to right, or, lowest seat id to highest seat id.
...
### SeatHold Class
- Data Structures:
  * **seats** is a LinkedList that I used to store all the seat Id's aligned to a particular seatHold. I chose a LinkedList because of its constant insertion time and O(n) space requirements. The O(n) search time does not negativley effect me since I only ever need to access the elements of **seats** in a linear fashion or by retreiving only the first element.
